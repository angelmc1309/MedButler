    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        listview.setOnItemClickListener { parent: AdapterView<*>, view: View, position: Int, id: Long ->
            if (position == 0) {
                ...
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set1_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set1_gray")
                finish()
            }
            if (position == 1) {
                Toast.makeText(this@ModelListViewSettings, "you selected Settings model picture two", Toast.LENGTH_SHORT).show()
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set2_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set2_gray")
                finish()
            }
            if (position == 2) {
                Toast.makeText(this@ModelListViewSettings, "you selected Settings model picture three", Toast.LENGTH_SHORT).show()
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set3_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set3_gray")
                finish()
            }
            if (position == 3) {
                Toast.makeText(this@ModelListViewSettings, "you selected Settings model picture four", Toast.LENGTH_SHORT).show()
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set4_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set4_gray")
                finish()
            }
            if (position == 4) {
                Toast.makeText(this@ModelListViewSettings, "you selected Settings model picture five", Toast.LENGTH_SHORT).show()
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set5_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set5_gray")
                finish()
            }
            if (position == 5) {
                Toast.makeText(this@ModelListViewSettings, "you selected Settings model picture six", Toast.LENGTH_SHORT).show()
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set6_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set6_gray")
                finish()
            }
            if (position == 6) {
                Toast.makeText(this@ModelListViewSettings, "you selected Settings model picture seven", Toast.LENGTH_SHORT).show()
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageId("set7_im")
                MainController.getUsuariPerProvarEdu().getappearanceInfo().setsettingsImageBackgroundId("set7_gray")
                finish()
            }
        }
    }
